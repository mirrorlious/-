...
- `npm test`：5 个 Vitest 文件、20 条单元测试、无障碍 ESLint、生产构建和 12 条 Microsoft Edge E2E 全部通过。
- `npm audit --audit-level=low`：0 个漏洞。
- `git diff --cached --check`：通过。
- 远端 `main` 与功能分支原为无共同祖先的历史；本次合并保留了 `main` 独有公共资料包，并以通过完整测试的 `feat/reader-annotations-v2` 版本处理同名阅读器文件。
